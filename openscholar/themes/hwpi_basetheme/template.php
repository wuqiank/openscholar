 ...
 *
 * @see os_basetheme_pager_link()
 */
function hwpi_basetheme_pager_link($variables) {
  $text = $variables['text'];
  ...
  if (!$rel_prev && $text == t('‹ previous')) {
    $rel_prev = TRUE;
    drupal_add_html_head_link(array('rel' => 'prev', 'href' => url($_GET['q'], array('query' => array_filter($query, '_hwpi_basetheme_not_empty')))));
  }
  if (!$rel_next && $text == t('next ›')) {
    $rel_next = TRUE;
    drupal_add_html_head_link(array('rel' => 'next', 'href' => url($_GET['q'], array('query' => array_filter($query, '_hwpi_basetheme_not_empty')))));
  }
 ...
 * @endcode
 */
function _hwpi_basetheme_not_empty($value) {
